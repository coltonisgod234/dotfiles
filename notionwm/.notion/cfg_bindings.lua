...
defbindings("WScreen", {
    ...
    kpress(ALTMETA.."minus", "ioncore.goto_prev_screen()"),
    bdoc("Go to next screen on multihead setup.", "->scr"),
    kpress(META.."minus", "ioncore.goto_next_screen()"),

    bdoc("Create a new workspace of chosen default type."),
    kpress(ALTMETA.."E", "ioncore.create_ws(_)"),

    bdoc("Display the main menu."),
    kpress(META.."Z", "mod_menu.menu(_, _sub, 'mainmenu', {big=false1})"),
    kpress(ALTMETA.."Z", "mod_query.query_menu(_, _sub, 'mainmenu', 'Main menu:')"),
...
defbindings("WClientWin", {
    ...
    kpress(ALTMETA.."colon", "WClientWin.quote_next(_)"),
})
...
defbindings("WGroupCW", {
    ...
    kpress_wait(META.."colon", "WGroup.set_fullscreen(_, 'toggle')"),
})
...
defbindings("WMPlex.toplevel", {
    ...
    kpress(META.."E", "mod_query.query_workspace(_)"),
...
defmenu("mainmenu", {
    ...
    menuentry("Firefox",	"mod_query.exec_on_merr(_, 'firefox')"),
    menuentry("Files",		"mod_query.exec_on_merr(_, 'pcmanfm')"),
    "separator",
    submenu("Utilities",	"apputils"),
    submenu("Internet",		"appinternet"),
    submenu("Programming",	"appcoding"),
    submenu("Multimedia",	"appmedia"),
    submenu("Audio",		"appaudio"),
    submenu("Games", 		"appgames"),
    submenu("System", 		"appsystem"),
    submenu("Design",		"appdesign"),
    submenu("Office",		"appoffice"),
    "separator",
    submenu("Session",          "sessionmenu"),
})

defmenu("apputils", {
	menuentry("Force Kill",	"mod_query.exec_on_merr(_, 'xkill')"),
})
defmenu("appinternet", {
	menuentry("Firefox",	"mod_query.exec_on_merr(_, 'firefox')"),
	menuentry("Discord",	"mod_query.exec_on_merr(_, 'vesktop 2>/dev/null')"),
})
defmenu("appcoding", {
	menuentry("VS-Code",	"mod_query.exec_on_merr(_, 'code')"),
	menuentry("Python REPL", "mod_query.exec_on_merr(_ 'kitty python')"),
	menuentry("Vim",	"mod_query.exec_on_merr(_ 'kitty vim')"),
	menuentry("Ghidra",	"mod_query.exec_on_merr(_, 'flatpak run org.ghidra_sre.Ghidra')"),
})
defmenu("appmedia", {
	menuentry("UxPlay", 	"mod_query.exec_on_merr(_, 'uxplay -fps 60')"),
	menuentry("Krita", 	"mod_query.exec_on_merr(_, 'flatpak run org.kde.krita')"),
	menuentry("Kdenlive",	"mod_query.exec_on_merr(_, 'flatpak run org.kde.kdenlive')"),
})

defmenu("appaudio", {
	menuentry("Amarok", 	"mod_query.exec_on_merr(_, 'flatpak run org.kde.amarok')"),
	menuentry("VLC",	"mod_query.exec_on_merr(_, 'vlc')"),
})

defmenu("appdesign", {
	menuentry("Draw.io",	"mod_query.exec_on_merr(_, 'flatpak run com.jgraph.drawio.desktop')"),
	menuentry("Blockbench",	"mod_query.exec_on_merr(_, 'flatpak run net.blockbench.Blockbench')"),
	menuentry("Bender",	"mod_query.exec_on_merr(_, 'flatpak run org.blender.Blender')"),
})

defmenu("appgames", {
	menuentry("Steam",	"mod_query.exec_on_merr(_, 'steam')"),
	-- Assumes you use osu-lazer-bin from the AUR
	-- If you use the flatpak version, change this command to `flatpak run sh.ppy.osu`
	-- If you use the appimage version, you're shit outta luck from me
	menuentry("Osu!Lazer",	"mod_query.exec_on_merr(_, 'osu-lazer')")
})

defmenu("appoffice", {
	menuentry("LibreOffice", "mod_query.exec_on_merr(_, 'libreoffice')"),
})
...
defmenu("sessionmenu", {
    ...
    menuentry("Restart into Fluxbox",    "ioncore.restart_other('fluxbox')"),
    menuentry("Logoff",           "ioncore.shutdown()"),
})
